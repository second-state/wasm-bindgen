...
impl<'a, 'b> Builder<'a, 'b> {
    ...
    pub fn process(
        ...
        for (i, param) in params.enumerate() {
            ...
            // Check param type whether supported
            if js.cx.config.mode.ssvm() {
                ssvm_type_check(&param, &arg)?;
            }
        ...
        // Check return type
        if js.cx.config.mode.ssvm() {
            for rt in &adapter.results {
                ssvm_type_check(&rt, "return value")?;
            }
        }
        ...
        if js.cx.config.mode.ssvm() {
            for (i, param) in adapter.params.iter().enumerate() {
                let arg = match explicit_arg_names {
                    Some(list) => list[i].clone(),
                    None => format!("arg{}", i),
                };
...
fn ssvm_type_check(at: &AdapterType, arg: &str) -> Result<(), Error>  {
    match at {
        AdapterType::I32
        | AdapterType::S8
        | AdapterType::S16
